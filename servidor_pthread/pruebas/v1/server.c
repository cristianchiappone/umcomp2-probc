...
void *atender_cliente(void *arg) {
    ...
    leido = read(conn, buff, sizeof buff);
...
int insq(char logs[max][100], int *rear, char data[100]) {
    if (*rear == max - 1)
        return (-1);
    else {
        *rear = *rear + 1;
...
int clearq(char logs[max][100], int *rear, int *front, char data[100]) {
    if (*front == *rear)
        return (-1);
    else {
        int log = open("log/1", O_APPEND | O_CREAT | O_WRONLY, 0777);
        if (log < 0) {
            perror("Open: ");
        }
