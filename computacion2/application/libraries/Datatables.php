    ...
    private function get_ordering()
    {
      ...
      $Data = $this->ci->input->post('columns');
        ...
        foreach ($this->ci->input->post('order') as $key)
				{
					if (!isset($this->custom_sort[$Data[$key['column']]['data']]))
						$this->ci->db->order_by($Data[$key['column']]['data'], $key['dir']);
					else
						$this->ci->db->order_by($this->custom_sort[$Data[$key['column']]['data']], $key['dir']);
				}
		}
    ...
    */
    ...
    public function last_query()
    {
      return  $this->ci->db->last_query();
		...
		/**
    * Cambia el orden que se usa para una columna
    *
    * @param string $column Columna a mostrar
    * @param string $column_sort Columna que se va a utilizar para el orden
    * @return mixed
    */
    public function custom_sort($column, $column_sort)
    {
      $this->custom_sort[$column] = $column_sort;
			return $this;
    }
  }
