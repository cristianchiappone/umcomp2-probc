...
bool validar_dispositivo(char *payload) {
    ...
    if (device_id->type == cJSON_String || device_id->type == cJSON_Raw) {
        ...
        sprintf(query, "SELECT * FROM dispositivo WHERE id = %s",
...
bool validar_payload(char *payload) {
    ...
    if (device_id->type == cJSON_String || device_id->type == cJSON_Raw) {
        ...
        sprintf(
            query,
            "SELECT tm.id as tipo_medicion_id,tm.codigo "
            "FROM dispositivo d "
            "join dispositivo_tipo_medicion dtm ON dtm.dispositivo_id = d.id "
            "join tipo_medicion tm ON tm.id = dtm.tipo_medicion_id "
            "where d.id = %s",
            device_id->valuestring);
        ...
        if (res->row_count == 0) {
            ...
            if (datetime != NULL) {
                sprintf(query_medicion,"INSERT INTO medicion VALUES(NULL,'%s','%s')",datetime,device_id->valuestring);
            }
                        ...
                        if (strcmp(field->name, "codigo") == 0) {
                            for (int j = 0; j < idx; j++) {
                                if (strcmp(row[1], sensor_name[j]) == 0) {
                                    ...
                                    sprintf(query_medicion_valor,"INSERT INTO medicion_valor VALUES(NULL,'%d','%s',%f)",medicion_id,row[0],sensor_value[j]);
